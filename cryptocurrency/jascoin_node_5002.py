...
def mine_block():
    ...
    blockchain.add_transactions(sender = node_address, receiver = 'gurmeet', amount = 1)
...
# checking for chain is valid or not
@app.route('/is_chain_valid', methods = ['GET'])
def is_chain_valid():
    is_valid = blockchain.is_chain_valid(blockchain.chain)
    message = 'We have a problem, Blockchain is not valid'
    if is_valid is True:
        message = 'Everything is fine, Blockchain is valid'
    response = {
        'message' :  message
    }
    return jsonify(response), 200
...
# replacing the chain with the longest_chain
